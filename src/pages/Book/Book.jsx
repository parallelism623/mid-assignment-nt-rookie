import { Card, Row, Col, Button, Input, Tag, Checkbox, Pagination } from "antd";

import { FiTag, FiLayers, FiXCircle, FiCheckCircle } from "react-icons/fi";
import { FaBook } from "react-icons/fa";

import { IoFilterSharp } from "react-icons/io5";
import { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router";
import FractionalStar from "../../components/ui/icons/FractionalStar";
import { bookServices } from "../../services/bookServices";
import BookBorrowModal from "./book-borrow-modal";
import BookFilter from "./book-filters";
import "../../assets/styles/BookStyle.css";
import { defaultQueryParameters } from "../../constants/queryParameters";
import { environment } from "../../constants/environment";
import { useMessageContext } from "../../components/context/MessageContext";
import ReviewButton from "../../components/ui/buttons/ReviewButton";
import EditButton from "../../components/ui/buttons/EditButton";
import DeleteButton from "../../components/ui/buttons/DeleteButton";
import ViewDetailButton from "../../components/ui/buttons/ViewDetailButton";
import BookReviewCardModal from "./book-review-card-modal";
import { useUserContext } from "../../routes/ProtectedRoute";
import ConfirmModal from "../../components/ui/ConfirmModal";
import { routesPath } from "../../constants/routesPath";
const { Search } = Input;
const MAX = environment.limit_books_per_request;

const Book = () => {
  const [queryParameters, setQueryParameters] = useState({
    ...defaultQueryParameters,
    available: false,
    rating: 0,
    categoryIds: null,
    pageSize: 8,
  });
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [openFilter, setOpenFilter] = useState(false);
  const [borrowMode, setBorrowMode] = useState(false);
  const selectedRowKeysRef = useRef([]);
  const [selectedRowKeys, setSelectedRowKeys] = useState(
    selectedRowKeysRef.current
  );

  const [openBorrowBookModal, setOpenBorrowBookModal] = useState(false);
  const [openBookReview, setOpenBookReview] = useState(false);
  const [selectedBook, setSelectedBook] = useState(null);
  const { message } = useMessageContext();
  const { id, roleName, refreshUser } = useUserContext();
  const [openDeleteConfirmPopup, setOpenDeleteConfirmPopup] = useState(false);
  const navigate = useNavigate();

  const fetchData = (isSetData = true) => {
    setLoading(true);
    const bookGetParams = {
      pageSize: queryParameters.pageSize,
      pageIndex: queryParameters.pageIndex,
      search: queryParameters.search,
      rating: queryParameters.rating,
      availability: queryParameters.available,
    };
    if (!!queryParameters.categoryIds)
      bookGetParams.categoryIds = queryParameters.categoryIds;
    bookServices
      .gets(bookGetParams)
      .then((res) => {
        if (isSetData) {
          setData(res.items);
          setQueryParameters({
            ...queryParameters,
            pageIndex: res.pageIndex,
            pageSize: res.pageSize,
            totalCount: res.totalCount,
          });
        }
        setLoading(false);
      })
      .catch(() => setLoading(false));
  };
  useEffect(() => {
    let isActive = true;
    console.log(queryParameters);
    fetchData(isActive);
    return () => {
      isActive = false;
    };
  }, [
    queryParameters.pageIndex,
    queryParameters.pageSize,
    queryParameters.search,
    queryParameters.totalCount,
    queryParameters.available,
    queryParameters.rating,
    queryParameters.categoryIds,
  ]);
  const actionOfBookItemsUser = (record) => [
    <ReviewButton
      size={15}
      onClick={() => {
        handleOnClickReviewBookButton(record);
      }}
    />,
    <ViewDetailButton
      size={15}
      onClick={() => navigate(routesPath.bookDetail.replace(":id", record.id))}
    />,
  ];
  const actionOfBookItemsAdmin = (record) => [
    <EditButton size={15} onClick={() => navigate(`${record.id}/edit`)} />,

    <DeleteButton
      size={15}
      onClick={() => {
        setSelectedBook(record);
        setOpenDeleteConfirmPopup(true);
      }}
    />,
    <ViewDetailButton
      size={15}
      onClick={() => navigate(routesPath.bookDetail.replace(":id", record.id))}
    />,
  ];

  const handleChange = (page, pageSize) => {
    setQueryParameters({
      ...queryParameters,
      pageIndex: page,
      pageSize: pageSize,
    });
  };

  const onSearch = (value) => {
    setQueryParameters({ ...queryParameters, search: value });
  };

  const handleOnConfirmDeleteBook = (record) => {
    setLoading(true);
    bookServices
      .deleteById(record.id)
      .then(() => {
        const newTotal = queryParameters.totalCount - 1;
        setQueryParameters({
          ...queryParameters,
          totalCount: newTotal,
          pageIndex:
            newTotal % queryParameters.pageSize === 0
              ? Math.max(1, queryParameters.pageIndex - 1)
              : queryParameters.pageIndex,
        });
        setLoading(false);
      })
      .catch(() => setLoading(false));
    setOpenDeleteConfirmPopup(false);
    setSelectedBook(null);
  };

  const toggleBorrowMode = () => {
    setBorrowMode(!borrowMode);
    if (borrowMode) {
      setSelectedRowKeys([]);
      selectedRowKeysRef.current = [];
    }
  };
  const handleSubmitBorrow = () => {
    if (selectedRowKeys.length) setOpenBorrowBookModal(true);
    else setBorrowMode(false);
  };
  const handleOnBorrowBooksChange = (newKeys) => {
    setSelectedRowKeys(newKeys);
    selectedRowKeysRef.current = newKeys;
    if (newKeys.length >= MAX) {
      message.info("You reach limit books per request");
    }
  };

  const toggleSelect = (id) => {
    const isSel = selectedRowKeys.includes(id);
    const newKeys = isSel
      ? selectedRowKeys.filter((k) => k !== id)
      : [...selectedRowKeys, id];
    handleOnBorrowBooksChange(newKeys);
  };
  const handleOnCancelBookReview = () => {
    setOpenBookReview(false);
    setSelectedBook(null);
  };
  const handleOnClickReviewBookButton = (book) => {
    setOpenBookReview(true);
    setSelectedBook(book);
  };
  const handleOnSubmitBookFilter = (values) => {
    setQueryParameters({
      ...queryParameters,
      pageIndex: defaultQueryParameters.pageIndex,
      ...values,
    });
  };
  return (
    <>
      {openDeleteConfirmPopup && (
        <ConfirmModal
          title={"Confirm Book Deletion"}
          message={`Are you sure you want to delete the book ${selectedBook.title}? This action cannot be undone`}
          visitable={openDeleteConfirmPopup}
          onCancel={() => {
            setOpenDeleteConfirmPopup(false);
            setSelectedBook(null);
          }}
          onSubmit={() => {
            handleOnConfirmDeleteBook(selectedBook);
          }}
          submitText={"Submit"}
        />
      )}
      <BookFilter
        open={openFilter}
        onClose={() => setOpenFilter(false)}
        onSubmit={(vals) => handleOnSubmitBookFilter(vals)}
      />
      <BookBorrowModal
        isModalOpen={openBorrowBookModal}
        bookIds={selectedRowKeys}
        onSubmit={() => {
          fetchData();
          refreshUser();
        }}
        onCancel={() => {
          setSelectedRowKeys([]);
          selectedRowKeysRef.current = [];
          setBorrowMode(false);
          setOpenBorrowBookModal(false);
        }}
      />
      {openBookReview && (
        <BookReviewCardModal
          visible={openBookReview}
          onCancel={handleOnCancelBookReview}
          onSubmit={() => {
            fetchData();
          }}
          book={selectedBook}
          reviewerId={id}
        />
      )}
      <Card
        loading={loading}
        title="Book List"
        extra={
          <Row className="flex items-center gap-2">
            <Col>
              <Search
                placeholder="Search by title/author/category"
                onSearch={onSearch}
                style={{ width: 240 }}
                defaultValue={queryParameters.search}
                loading={loading}
              />
            </Col>
            {roleName === environment.adminRole && (
              <Col>
                <Button onClick={() => navigate("/books/create")}>
                  Create Book
                </Button>
              </Col>
            )}
            {roleName !== environment.adminRole && (
              <Col>
                {!borrowMode ? (
                  <Button onClick={toggleBorrowMode}>Borrow Books</Button>
                ) : (
                  <Button onClick={handleSubmitBorrow}>
                    {selectedRowKeys.length ? "Submit Borrow" : "Cancel Borrow"}
                  </Button>
                )}
              </Col>
            )}
            <Col
              className="flex items-center cursor-pointer"
              onClick={() => setOpenFilter(true)}
            >
              Filters <IoFilterSharp className="ml-1" />
            </Col>
          </Row>
        }
        className="book-list-card align-center"
      >
        {data && (
          <Row gutter={[16, 16]}>
            {data.map((record) => {
              const isAvailable = record?.available > 0;
              const isSelected = selectedRowKeys?.includes(record.id);
              const reachedMax = selectedRowKeys?.length >= MAX && !isSelected;
              const disabled = reachedMax || !isAvailable;
              const availStyle = {
                display: "inline-block",
                background: isAvailable ? "#F6FFEC" : "#FFF1F0",
                color: isAvailable ? "#52C41A" : "#FF4D4F",
                padding: "2px 8px",
                borderRadius: "5px",
                fontSize: "12px",
                minWidth: "24px",
                textAlign: "center",
              };

              return (
                <Col key={record.id} xs={24} sm={12} md={8} lg={6}>
                  <Card
                    hoverable
                    cover={
                      <img
                        alt={record.title}
                        src={
                          record?.imageUrlSigned ||
                          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ2gtz3pCsopRPlbimr8KOduu3nnEFxAzTdHw&s"
                        }
                        className="h-48 w-full object-cover"
                      />
                    }
                    extra={
                      borrowMode && (
                        <Checkbox
                          checked={isSelected}
                          disabled={disabled}
                          onChange={() => toggleSelect(record.id)}
                        />
                      )
                    }
                    actions={
                      roleName === environment.adminRole
                        ? actionOfBookItemsAdmin(record)
                        : actionOfBookItemsUser(record)
                    }
                    className={`rounded-xl shadow-lg border border-gray-200 bg-white ${
                      isSelected ? "ring-2 ring-blue-400" : ""
                    }`}
                  >
                    <Card.Meta
                      description={
                        <>
                          <h1 className="!text-gray-700 text-lg">
                            {record.title}
                          </h1>
                          <p className="text-sm text-gray-600">
                            by {record.author}
                          </p>
                        </>
                      }
                    />
                    <div className="mt-4 space-y-2">
                      <div className="flex items-center space-x-2">
                        <FiTag className="text-gray-500" />
                        <span className="font-medium text-gray-700">
                          Category:
                        </span>
                        <Tag color="blue">{record.category.name}</Tag>
                      </div>
                      {roleName === environment.adminRole && (
                        <div className="flex items-center space-x-2">
                          <FiLayers className="text-gray-500" />
                          <span className="font-medium text-gray-700">
                            Quantity:
                          </span>
                          <Tag color="yellow">{record.quantity}</Tag>
                        </div>
                      )}

                      <div className="flex items-center space-x-2">
                        {isAvailable ? (
                          <FiCheckCircle className="text-green-500" />
                        ) : (
                          <FiXCircle className="text-red-500" />
                        )}
                        <span className="font-medium text-gray-700">
                          Available:
                        </span>
                        <span style={availStyle}>
                          {isAvailable ? record.available : "Invailable"}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <FractionalStar
                          rating={record.averageRating.toFixed(1)}
                          size={18}
                        ></FractionalStar>
                        <span className="font-medium text-gray-700">
                          Average rating:
                        </span>
                        <span className="text-gray-800 font-semibold">
                          {record.averageRating.toFixed(1)} (
                          {record.numberOfReview} reviews)
                        </span>
                      </div>
                    </div>
                  </Card>
                </Col>
              );
            })}
          </Row>
        )}
        {data?.length === 0 && (
          <>
            <div className="m-auto text-gray-300 w-fit">
              <FaBook key="no-data" className="size-10 m-auto" />

              <div className=" text-gray-300">No Data</div>
            </div>
          </>
        )}
        <div className="mt-6 flex justify-end">
          <Pagination
            current={queryParameters.pageIndex}
            pageSize={queryParameters.pageSize}
            total={queryParameters.totalCount}
            showSizeChanger
            pageSizeOptions={[4, 8, 12, 20, 40, 100]}
            onChange={(p, ps) => handleChange(p, ps)}
            showTotal={(t, [start, end]) => `From ${start} to ${end} of ${t}`}
          />
        </div>
      </Card>
    </>
  );
};

export default Book;
